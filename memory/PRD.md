...
- ✅ **🆕 Session 82 — Three-tier Pricing section on landing (Feb 23 2026) — UPDATED for mobile side-by-side carousel**:
  ...
  - **🆕 Mobile horizontal-snap carousel (Feb 23 2026 — patch)**: user requested that on mobile the 3 cards stand **side-by-side** instead of stacking vertically (a 375px-wide phone can't fit 3 full readable cards in a row). Implementation: cards rail switches from `grid grid-cols-3` (md+) to `flex overflow-x-auto snap-x snap-mandatory` (mobile), each card wrapped in `w-[88vw] snap-center shrink-0` so one card is visible at a time with a small peek of the next/prev card as a swipe affordance. JavaScript: on mount the rail auto-centers the Premium card (idx 1, the "MOST POPULAR" tier) so users see the recommended option first; a `scroll` listener tracks the centred card and powers a 3-dot indicator below the rail with text "Swipe to compare plans". Verified on iPhone-sized viewport (390×844): `display:flex`, `overflowX:auto`, `scrollSnapType:x mandatory`, scrollWidth=1110 / clientWidth=342 (real overflow), Premium centred on first paint, Free + VIP reachable by swipe.
