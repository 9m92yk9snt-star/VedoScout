...
- **Resultat 29.0–29.5 (6 frames, ægte pipeline-clip)**: ellipse på den KORREKTE hvide spiller i alle frames (standfod/ankel-niveau), ingen trailing til mudder/linje, ingen snap til blå modstander/hvid holdkammerat, ærlig fade ved vinduets slutning. BESTÅET.
- **Regression uændret**: duel 49.0 (fødder inde, okklusion) ✓, fald 49.53 (anker på kontaktpunkt) ✓, iter95-suite 10/10 ✓.
- **Endelig regenerering (3 rapporter, shipping-pass)**: frames 6/3/5 + clips 2/1/2 (alle 6.0s med clip_start/end) re-renderet med den ENDELIGE resolver.
- **Vigtig testviden**: track-boksen kan ligge helt under spilleren ved sprint (NCC-låst på linje/græs) — det er A-rodårsagen; produktionen beskyttes af opad-redning + skjul + GPT-kantvagt. Track-data selv fortsat urørt.
- Filer: tele_clip.py alene. IKKE RØRT: taps, Re-ID, identity-gates, Analysis A/B, evidence-logik, player_tracking.py.
- ⚠️ KLAR TIL DEPLOY (bruger trykker Deploy) → derefter regenerate-evidence på produktionsrapporter.

## Session (Jun 2026 — part 26) — DE 2 SIDSTE HULLER FØR DEPLOY: A-FIX (sprint) + C-FIX (padding) ✅ (self-tested + syntetisk verifikation + regression 10/10 + afsluttende regenerering)
- **A-FIX (visuelt lag, tracking urørt)**: `_ground_anchor(..., vx=)` — track-hastigheden (målt fra selve tracket, _interp ±0.15s → px/s) LEDER søgefeltet i bevægelsesretningen (lead = clip(vx·0.25, ±0.55·bw)); ownership-filter og plausibilitets-klemme skifter med. Anker snapper stadig KUN til ægte pixels — intet fast offset. + Sprint-responsiv EMA: k = 0.35→0.85 skaleret med anchor-bevægelse (hurtig konsistent bevægelse følges tæt = ingen trailing; jitter dæmpes; >0.9·bh = glide 0.12). Syntetisk verificeret: boks 40px bag spiller + vx → anker 199.5/200; uden vx → ærlig fallback; duel-ejerskab holder.
- **C-FIX (padding i stedet for fuld-video-fallback)**: `MIN_TRACKED_SEC=1.2` (mindste verificerede kerne), clip PADDES med rå kontekst til ≥MIN_CLIP_SEC 3.0 via PAD_PRE 2.5/PAD_POST 3.5 omkring eventet — markøren tegnes KUN i det verificerede vindue (alpha 0 udenfor; ring-fade ved vinduets kanter). Padding hævder aldrig identitet. Fuld video KUN når kilden selv er < MIN_CLIP_SEC+1s. Return har nu start/end (klip) + tracked_start/end. `res["start"]` = padded clip-start → THE MOMENT-aktivering (sec − clip_start) forbliver præcis.
- **Afsluttende regenerering (3 rapporter)**: d8c04d5d 2 clips (32.02–38.02, 47.38–53.38), fe7bc3e5 1 clip (63.94–69.94), 3467a622 2 clips (16.75–22.75, 23.15–29.15) — ALLE 6.0s med kontekst→moment→fuldførelse, clip_start/end persisteret, frames 6/3/5 re-renderet med motion-aware anker. GPT-kantvagt kørte (shrink-loops observeret = korrekt gating af markør-vinduet).
- **Visuel accept**: pad-zone frame = rå video UDEN ring; tracked-zone frame = fald-casen med ellipse på faktisk kontaktpunkt, modstander-ben krydser uden ejerskabsskifte.
- **Regression**: iter95-suiten 10/10 (clips OPTIONAL per rapport, ≥1 på tværs med metadata — nu 5 clips).
- **IKKE RØRT**: taps, Re-ID, identity-gates, Analysis A/B, evidence-logik, player_tracking.py.
- Filer: tele_clip.py alene (konstanter, _window_points-gate, _ground_anchor vx-lead, _draw_ring adaptiv k + vx, generate_tracked_clip padding/v0v1/vx).
- ⚠️ REQUIRES REDEPLOY → derefter regenerate-evidence på produktionsrapporter.

## Session (Jun 2026 — part 25) — ÉN DIAGNOSE-PAS → MINIMALE ROOT-CAUSE FIXES (A/B/C) ✅ (self-tested + regression 10/10 + én regenererings-pas)
- **A Track-geometri (DIAGNOSE)**: Identitet KORREKT (tap-seeded NCC + farve-veto stopper ærligt). Geometrien trailer under sprint (29.0–29.5s: boks x≈0.37 statisk, conf 0.68–0.83, track slutter 29.5 → shadow-TELEPORT@29.8 evaluerede stale punkt). Tracker IKKE ændret (beskyttet produktionsautoritet; risikovinduer er allerede gated ud af evidence). Synligt symptom fixet i anker-laget (B).
- **B Grounded ellipse (FIX — kun renderer)**: NY `_ground_anchor()` (Visual Ground-Anchor Resolver) i tele_clip.py, kaldt i toppen af `_draw_ring` → virker automatisk for clips + statiske frames + GPT edge-crops. Finder FAKTISK kontakt (fod/hånd-band = laveste silhuet-pixels), søger op til 45% UNDER boks-bund (fald/crouch), ownership-klemme til boksens egne kolonner (dueller — ellipse skifter aldrig ejer), stance-adaptiv bredde fra fodaftryk (foot_w·0.85, cap est_h·0.50), temporal EMA k=0.45 + glide ved implausible hop (>0.6·bh) i clips via `state`-dict. Fail-safe fallback = bbox-bund-center (<25 mask-px, |Δx|>0.45·bw). INGEN hardcodede offsets. Visuelt verificeret: duel 49.0 (fødder inde), fald 49.53 (anker på kontakt), wide-box crossover 11.4.
- **C Proof/THE MOMENT (FIX)**: (1) "PLAYER TRACKED"-proof-mode FJERNET — clip-badge er nu "Moment at"/"The moment" med aktivering ved `event_start − clip_start` (±0.15s/+2s vindue); (2) `tele_clip_start`/`tele_clip_end` persisteres på video_comments (server 7719ff + pops ved reset), flyder gennem ensure_video_frames/serializer, bruges i proofplayer.jsx via PremiumReportV2 playAt; (3) MIN_CLIP_SEC 1.4→3.0 (ingen meningsløse 2–3s loops — for korte verificerede vinduer ⇒ fuld-video-fallback med korrekt moment-chip); (4) default pre/post 1.8/1.8→2.5/3.5 (kontekst → MOMENT → fuldført aktion; GPT-kantvagt trimmer stadig usikre kanter); (5) build-up-noten bruger nu ægte clip_start for clips.
- **ÉN regenererings-pas (3 rapporter)**: frames 6/3/5 re-renderet med nyt anker (GPT-gated 5/3/5 — 1 frame korrekt afvist), clips: d8c04d5d 0 (montage: alle vinduer 0.5–2.6s — ærligt fravalg), fe7bc3e5 1 clip 63.96–67.88 (3.9s, start/end persisteret, R2 200), 3467a622 0 (GPT-kantvagt afviste: "ellipse under nr. 8, ikke 15" / "empty grass" — fail-safe). R2-signering OK.
- **VIGTIG TESTVIDEN**: Headless Chromium (Playwright) mangler H.264 → `DEMUXER_ERROR_NO_SUPPORTED_STREAMS` på ALLE mp4-clips i UI-tests. Serverede bytes er md5-identiske med valid h264/yuv420p-fil. IKKE en produktfejl (rigtige browsere spiller fint; fallback-flow virker endda pænt ved fejl). Brug ikke clip-afspilning i headless som fejlkriterium.
- **Regressionstest opdateret**: iter95-suiten — clips nu OPTIONAL per rapport (≥3s-regel); kræver ≥1 clip på tværs af seeded reports MED tele_clip_start/end metadata. 10/10 pass.
- **IKKE RØRT**: taps, Re-ID, identity-gates, Analysis A/B, scores, konklusioner, evidence-verifikation, player_tracking.py, cv_shadow.
- Filer: tele_clip.py (+_ground_anchor, state, MIN 3.0), server.py (pre/post + clip_start/end persist/pop), proofplayer.jsx (moment-aktivering, badge, note), PremiumReportV2.jsx (clipStart), tests/test_iter95_cv_shadow.py.
- ⚠️ REQUIRES REDEPLOY. Produktionsrapporter (fx Ormani3) får nye assets først ved regenerate-evidence dér.

## Session (Jun 2026 — part 24) — GROUND-INTEGRERET MARKØR (kun visuel rendering) ✅ (self-tested: renders på verificerede momenter + zoom-inspektion + clip-generering)
- **Bruger-brief**: ellipsen lignede stadig et grafisk overlay. KUN visuel rendering ændret — tracking/identity/taps/Analysis A/B/timestamps/gates urørt.
- **ÉN delt renderer**: `tele_clip._draw_ring()` totalt omskrevet til "painted-on-grass"; `telestration.render_telestration` kalder nu SAMME funktion (PIL→numpy→PIL) — frames og clips kan aldrig divergere visuelt. Gammel PIL-ring + separat occlusion-blok i telestration slettet.
- **Nyt look**: (1) volt-farven moduleres af græssets luminans (0.42+0.58·gray) = kridt/maling der arver banens tekstur — ikke flad neon; (2) blød contact shadow under spilleren (blurred ellipse, 28% mørkning — restaureres væk PÅ spilleren, fringen bliver på græsset); (3) fill 10%→7% med blød kant; (4) glow 45%→20%, smallere; (5) ring-linje 92%→80% alpha med gaussian edge-softness (σ=lw·0.35) + offset dark under-stroke (26%) for dybde; (6) perspektiv: rh/rw = 0.26+0.14·(y/H) — fladere langt væk, rundere tæt på; (7) bred stance: rw udvides op til min(bw·0.55, est_h·0.45) så BEGGE fødder er inde; (8) occlusion-bånd udvidet ±0.6rw→±0.95rw + 3×3 dilation (dræber anti-alias halo) + feather 5→7.
- **Stabilitet**: EMA (k=0.25) på boks-STØRRELSE i clip-loopet (position forbliver responsiv via eksisterende ±2-sample smoothing) — ingen størrelses-vibration.
- **Verificeret**: statisk + clip renderet lokalt på verificerede evidensmomenter (49s, d8c04d5d; clip coverage 1.0) — zoom-inspektion bekræfter: fødder inde, ring skjult bag støvler/ben (også modstanderes), malet look, ingen label. Test ved 36s viste ringen følge track-drift (36.4s = kendt teleport-flag) = bekræfter ren downstream; det moment er i forvejen gated i produktion.
- **Genbrugstest**: `tests/render_marker_test.py {t}` (ingen DB-writes, ingen LLM). INGEN regenerate-evidence kørt (sparer GPT-kredit) — eksisterende rapporters assets har stadig det gamle look indtil regenerering; alle NYE renders får automatisk det nye look.
- ⚠️ REQUIRES REDEPLOY.

## Session (Jun 2026 — part 23) — DE 5 DELVISE FASER FÆRDIGBYGGET I SKYGGE (engine v6) ✅ (self-tested: synth-tests + reel kørsel på alle 3 rapporter + iter95-regression 10/10 + UI-screenshot)
- **Fase 8 Kamera-komp (fuld)**: NY `CameraMotion` i cv_detect.py — goodFeaturesToTrack + LK optical flow + estimateAffinePartial2D (RANSAC) = pan/zoom/rotation; fallback phaseCorrelate; fail-safe none. Bruges i exp-kontinuitet, fysik-gate og teleport-gate via `cam.point()`. Reelt: affine 367/383, 348/365, 148/148 frames (fallback klumper ved scene cuts = korrekt).
- **Fase 3/4 Re-ID + feature ownership**: `_zone_embedding(..., occluders=)` fjerner okkluderende personers pixels fra masken (owned=True/False/None; <30% synlig ⇒ tagged contaminated, aldrig gættet). Anvendt på tap-refs, prod-boks-eval, switch-konkurrenter og kandidater. `_profile_sim` foretrækker nu SAMME-skala referencer i udvælgelsen (bonus kun i sortering — straffer ALDRIG sim-værdien). Reelt: 43/3, 30/2, 21/7 owned/contaminated.
- **Fase 7 Persistent negativ-galleri**: `_add_negative()` dedupe (sim>0.92 skippes) så 30-cappen dækker DISTINKTE teammates; NY prod-signal `neg_match_frames`/`neg_ts` — kendt teammate matcher prod-boks-indholdet klart bedre end target (2 samples persistens). Reelt: d8c 2 (ved kendt 11.2–12.2s crossover), 346 1.
- **Fase 9 Teleport-gate på PROD-tracket**: kamera-kompenseret hop > MAX_SPEED*1.6 OG > 0.8*med_h inden for dtp≤0.6s ⇒ `teleport_frames`/`teleport_ts`; scene cuts ekskluderet (kun cam.mode=affine). Reelt 9/15/5 — visuelt bedømt: 36.4s = kendt risikomoment, 71.4–72.2s = kendt track-ende-klasse, ÆGTE fund. OBS: raten (7–15%) skal kalibreres på flere kampe før evt. gate-brug.
- **Fase 13 Sikker re-akkvisition (multi-signal)**: reacq_pending state — REACQ_BONUS-gate holder gennem HELE re-akkvisitionen; streak af KONSEKUTIVE rene samples (reset ved uren), 3 kræves når tabt i crowd (lost_in_crowd huskes ved LOST-transition); team-veto (opponent/other ⇒ UNCERTAIN + reacq_team_rejected); pose-relaxed sim ≥ SIM_T kræves ved VERIFIED-promotion. Metrics `reacq {attempts, verified, team_rejected, max_streak_required}`.
- **Resultater alle 3 rapporter (v6 gemt)**: suspect 1/1/1 (aggregat 3/295 = 1.0%), switch 0/1/0 (0.3% ≤ 2%-reglen — gap bridging FORBLIVER aktiv), tap_self 0.818/0.778/0.75, agreement 1.0. NYT switch-flag fe7bc3e5@21.4s visuelt bedømt ÆGTE (prod-boks driver i sprint-klynge, samme window som empty 21.1–21.7 — allerede beskyttet nedstrøms).
- **iter95-regressionstest opdateret**: hård `switch==0` → rate ≤ 0.02 (samme dokumenterede grænse som gap-bridge-aktivering). 10/10 pass: ingen cv_shadow-læk til købere, admin-auth, media 200, gap bridge aktiv.
- **Admin-dialog**: nye Stats (Teammate-match/Teleport, warn>0), galleri-kinds NEG (rosa)/TELEPORT (blå), footer: camera affine/owned evals/re-acq. Screenshot-verificeret.
- **INTET produktionsflow ændret**: tracker, Analysis A/B, evidens-gates, proof clips, marker urørt. Alle nye signaler er skygge-metrics. Filer: cv_shadow.py, cv_detect.py, CvShadowDialog.jsx, tests/test_iter95_cv_shadow.py.
- ⚠️ REQUIRES REDEPLOY (skyggen kører kun v6 i preview indtil da).
...
- **Skygge-falsk-positiv fixet**: YOLO missede små/fjerne spillere → falske empty-flags. Empty kræver nu OGSÅ ikke-personligt boks-indhold: non-green fraktion <0.55 (målt: buske/baggrund 0.35–0.52, ægte spiller 0.65–0.86).
- **Valideret + persisteret på alle 3 rapporter** (tracks re-genereret med fixet tracker + shadow re-kørt): AGGREGATE suspect 3.4%→**0.7%**, switch 3→**0**, empty pr. rapport 32→5 / 25→13 / 18→1. Visuelt bekræftet: nyt track følger spilleren korrekt ved 69–70.9s hvor gammelt track stod på buske. Nye doubts viser ærlige stop (scene cut 16.7/25.5, static lock 1.7/24.2).
- Nye scripts: `tests/retrack_reports.py` (re-track + persist), `tests/run_tracker_fix_check.py` (old-vs-new sammenligning). OBS: heredoc med `$set` i bash -c æder dollartegn — brug scriptfiler.

## Session (Jun 2026 — part 18) — CROSS-REPORT VALIDERING DOKUMENTERET + GAP BRIDGING AKTIVERET ✅
- **Valideringsbasen voksede af sig selv**: brugeren uploadede 2 nye rigtige videoer; produktions-hooket kørte skyggen automatisk (engine v5). Basen nu 3 rapporter / 328 track-tjek: aggregate suspect 3.4%, switch 0.9%, sim_mean 0.569. Nye rapporter har 1-2 suspects / 0-1 switch → v5-positurrobusthed generaliserer. Empty-box klumper igen ved track-ender (18/25/32) — konsistent fejlklasse på tværs af kampe (beskyttet af unsafe-vinduer).
- **Gap bridging AKTIVERET med brugerens regel**: `_cv_gap_bridge_allowed()` — kræver ≥`CV_GAP_BRIDGE_MIN_REPORTS` (2) shadow-rapporter OG aggregate suspect ≤ `CV_GAP_BRIDGE_MAX_SUSPECT` (0.15) OG switch-rate ≤ `CV_GAP_BRIDGE_MAX_SWITCH_RATE` (0.02). Opfyldt nu → ACTIVE. `cv_shadow.bridge_track_points()` interpolerer KUN inde i pre-validerede sikre vinduer (identity begge sider + ingen crowd/empty + fysik) og kun i RENDERING-kopien af tracket (gemt player_track ALDRIG muteret; punkter mærket bridged, conf×0.9). Flag `CV_GAP_BRIDGE_ENABLED` default on. Anvendes i `_generate_tele_clips`.
- **Fail-safe bekræftet**: reglen er aktiv men per-gap-sikkerheden giver 0 bridgeable vinduer på nuværende videoer (kanter har crowd/empty-kontekst) — udfyldning sker først når en gap reelt ER sikker. fe7bc3e5 mangler gap_bridging-felt (kørte før part-16-feltet) — kommer automatisk på fremtidige kørsler.
- Admin stats-panel viser nu "gap bridging ACTIVE/dormant" (testid cv-gap-bridge-status). Summary-endpoint udvidet.
- Unit-tests: bridge-interpolation (5 punkter i 1.2s hul, conf×0.9), no-windows→uændret objekt.

## Session (Jun 2026 — part 17) — PHASE 17 HÅRD EVIDENS-GATE + PHASE 18 FLERE UNIKKE PROOFS ✅ (unit + stub-E2E på ægte shadow-data, 0 ekstra LLM-kredit)
- **Phase 17 hård gate**: (a) `score_meaning._pick_evidence` — identity-usikre momenter (±1.5s fra switch-risk / inde i empty-vinduer) EKSKLUDERES helt som kort-evidens; er ALLE citerede momenter usikre → kortet viser ingen proof (None). Flag `CV_EVIDENCE_HARD_GATE` default on. (b) `_generate_tele_clips` — clip afvises hvis nøglemomentet ligger i et identity-unsafe vindue ("uncertain player + obvious action → reject"). Unit-testet (safe-alternativ vælges, all-risky→None, uden risk uændret).
- **Phase 18 flere unikke proofs**: cap 3 → `TELE_CLIP_MAX_STABLE` (6) NÅR shadow-stabilitet er dokumenteret: `suspect_rate ≤ CV_PROOF_EXPAND_MAX_SUSPECT` (0.10) OG `switch_risk_frames ≤ CV_PROOF_EXPAND_MAX_SWITCH` (2) — env-tunbare. Uniqueness-regel: nye clips skal være >3s fra allerede clippede momenter (aldrig near-duplicates). Hvert clip stadig GPT-kant-verificeret individuelt (uændret).
- **Stub-E2E på d8c04d5d (ægte DB/shadow-data, mocket GPT+encode = 0 kredit)**: cap 6 aktiveret (real suspect_rate 0.052), 68.0s afvist (ægte unsafe-vindue 66.5–69.9), 6.0s skippet som duplikat af 5.0, 6 unikke clips. Logs bekræfter alle tre gates.
- OBS: rigtige nye rapporter får automatisk hele kæden (shadow → vinduer → gates → op til 6 unikke verificerede proofs). Eksisterende rapporter påvirkes først ved regenerering.
...
- **Hard-reject**: tap med consistency < median−0.30 OG quality < 0.5 → weight 0 (`rejected`), max 2, aldrig ved <5 taps. Outlier-downweight (0.3×) bevaret.
- **Apples-to-apples prod-verifikation**: prod-boksen evalueres via den overlappende DETEKTION (IoU>0.2) når muligt — fjernede kunstige suspects fra løs prod-boks vs stramme referencer (sim_mean 0.483→0.528).
- **Galleri-dedup**: max ét billede pr. hændelse pr. kind (1.5s afstand) så 12-cappen dækker distinkte fund.
- **Resultater d8c04d5d**: tap_self_verify 0.727→**0.818**; crossover-tappet 11.49s korrekt outlier (w 0.29); alle resterende flags klumper i 67.2–68.6s. **VISUELT BEKRÆFTET ÆGTE PROD-FEJL: ved 68.4s står prod-boksen (conf 0.998!) på tomme buske mens spillet er i modsatte side** — skyggen fanger hvad prod-konfidensen aldrig ville.
- **NÆSTE FASE (brugerens nummerering)**: Phase 2 Multi-View/Multi-Scale Re-ID = EKSISTERER allerede (multi-scale zoner close/medium/far). Phase 3 (detection+MOT), 4 (team), 5 (negativ-galleri), 6 (kamera-komp), 7 (fysik-gate), 8 (crossover-freeze), 9 (margin), 10 (states), 11 (re-acquisition), 12 (safeguards bevaret), 14 (marker states), 16 (dual analysis), 19 (shadow validation) = EKSISTERER i cv_shadow/cv_detect. REELT TILBAGE: Phase 13 (marker foot-anchor/ellipse ER bygget — evt. finpudsning), Phase 15 (context-aware gap bridging — MISSING), Phase 17 (hård evidens-gate — kun soft-gate findes), Phase 18 (flere proofs — venter på stabil tracking). Anbefalet næste: Phase 15 gap bridging i skygge ELLER hård track-ende-trimning (68.4s-fejlklassen).

## Session (Jun 2026 — part 14) — 4 PAKKER: positur-robusthed, tværs-statistik, P7 soft evidens-gate, P19 marker state-fade ✅ (alle self-tested E2E)
- **cv_shadow.py engine v5 (positur-robusthed)**: `_sim_relaxed()` (pooled helkrops-stats, ingen zone-orden/aspekt) redder bøjede/faldne positurer — MEN kun når en detekteret person faktisk ER i prod-boksen; flags kræver 2 konsekutive lave samples; switch-risk kræver samme-kit-konkurrent (opponent/other filtreres). NY metrik `empty_box_frames`+`empty_ts` (+EMPTY-BOX galleri-billeder, lilla i UI): detektor finder INGEN person i prod-boksen (2 samples persistens). Resultat på d8c04d5d: suspect 10→0, switch 11→0 (12.2s-tilskuer-alarmen elimineret), MEN 32 empty-box flags fundet ved 0.4–2.4s + 66.8–72s — visuelt verificeret ÆGTE fund (prod-boks ved mål/tilskuere mens spillet er andetsteds). Ægte fejl-klassen fra tidligere (tom boks) fanges stadig.
- **Tværs-rapport-statistik**: `GET /api/admin/cv-shadow/summary` (aggregat + pr.-rapport rækker) + `CvShadowStats.jsx` panel øverst i admin Reports-tab (testids cv-shadow-stats-toggle/-panel/-agg/-row-{id}). Skygge kører automatisk på alle nye premium-uploads → statistik vokser af sig selv.
- **P7 soft evidens-gate (`score_meaning.py`)**: `_risky_seconds(doc)` (switch_ts fra cv_shadow, flag `CV_EVIDENCE_SOFT_GATE` default on) + `_pick_evidence(..., risky)` — momenter ±1.5s fra switch-risk vælges SIDST, droppes ALDRIG; tom risky-liste = identisk gammel adfærd (unit-testet inkl. legacy 3-args kald). Analysis A/B urørt.
- **P19 marker state-fade (`tele_clip.py` + server)**: `_risk_alpha()` fader ring+chip blødt ud inde i switch-risk-vinduer (±0.6s, FADE_SEC ramper). `generate_tracked_clip(..., risky_windows)`; server læser switch_ts FRISKT fra DB ved clip-generering (flag `CV_MARKER_STATE_FADE` default on; fail-open = gammel adfærd). Visuelt verificeret: ring synlig udenfor, helt væk inde i vindue.
- **OBS**: bd972050-rapporten er SLETTET fra DB (kun d8c04d5d + demo tilbage) — brug d8c04d5d til fremtidige tests. Statistik-basen er nu 1 rapport; flere rigtige uploads behøves før gates aktiveres hårdt.
...
- **cv_shadow.py engine v4**: detektioner erstatter blobs som kandidater (blob-fallback), prod-verifikationens "others" fra ægte detektioner, P10 negativ-galleri høstes fra target-team-detektioner >2×med_h fra prod-boksen (0→30 negativer), P9 holdklassifikation som rank-PENALTY 0.6 (aldrig hård beslutning). Tap-refs gemmer nu chroma.
- **VALIDERET på bd972050**: compute 64s (baggrundstråd), 6.17 personer/frame, 446 MOT-tracks, 175 crossover-events, holdsplit 1954/287/99. **Visuel verifikation ved 11.9s: VÆLTET målspiller korrekt target_team (grøn), alle 3 blå modstandere korrekt opponent** — detektoren finder spillere blob-metoden missede. Sikkerhed holder: prod_agreement 1.0, potential_switches 0, switch_risk-tidsstempler stabile og matcher visuelt bekræftede risikomomenter (11.2/19.4/36/43/55.4s).
- **Kendt begrænsning**: holdsplit stadig target-tung (montage-video hvor kamera følger målspilleren + scene 2 har lyse kits på begge hold) — penalty-only design gør fejlklassifikation ufarlig. margin_median ~0 nu (negativ-galleri med identiske kits trykker margin — KORREKT fail-safe adfærd).
- Admin-dialog viser nu scene-metrics (testid cv-shadow-scene). Genbrugstest: `tests/run_cv_shadow_check.py`. ⚠️ REQUIRES REDEPLOY.

## Session (Jun 2026 — part 10) — CV IDENTITY SHADOW ENGINE (P0–P6) ✅ (self-tested på RIGTIG rapport + E2E admin-flow; testing agent BEVIDST ikke brugt endnu — brugerens instruks)
- **Bruger-brief**: stor additiv, feature-flagget, fail-safe CV-identitetsarkitektur RUNDT OM eksisterende system. INTET eksisterende ændret (upload, taps, tracking, Analysis A/B, evidens, proofs, marker urørt). Identitetssikkerhed > dækning; usikkerhed ⇒ LOST/UNCERTAIN, aldrig gæt.
- **`/app/backend/cv_shadow.py` (engine v3, ren OpenCV, 0 LLM-kredit, ~29s/rapport i baggrundstråd)**: P1 tap-refinement (blob nærmest tap-centrum), P2 multi-scale Re-ID (komponent-maskeret person-isolation + belysningsnormaliserede Lab-momenter — kalibreret: gamle HSV-hist var ubrugelig, baggrund/net/sol-skygge dominerede), P3-lite blob-scene, P4 tap-konsistens+outlier-downweight, P10 negativ-galleri, P11 kamera-komp (phaseCorrelate), P12 fysik-gate, P13 crossover-freeze, P14 margin-regel, P15 states (VERIFIED/PROVISIONAL/UNCERTAIN/LOST), P16 sikker re-akkvisition (2 rene samples). **HOVEDSIGNAL (v3): direkte PROD-BOKS-VERIFIKATION** — matcher indholdet af produktions-trackerens boks stadig tap-profilen? → prod_verify {checked, sim_mean, suspect_frames/rate, crowded, switch_risk} + tidsstempler (suspect_ts/switch_ts/crowded_ts).
- **Wire-in (server.py ~7975, efter `_tracking_core`)**: `asyncio.create_task` baggrundskørsel efter tracking, gated af `CV_SHADOW_ENABLED` (default on, env-flag). Gemmer KUN `report.cv_shadow` — `_serialize_report` er whitelist ⇒ lækker ALDRIG til købere (verificeret via API). Import ved linje ~14630.
- **Admin**: `GET /api/admin/reports/{id}/cv-shadow` (+`?run=1` on-demand, restorer video fra R2 hvis lokal mangler). NY `CvShadowDialog.jsx` + Radar-knap pr. premium-rapport i admin Reports-tab (testids: admin-cv-shadow-{id}, cv-shadow-dialog/-stats/-moments/-run-btn/-close/-empty).
- **VALIDERET på rigtig rapport bd972050 (11 taps, 330 track-punkter, 76.8s)**: tap_self_verify 0.27→0.73 efter kalibrering; prod_verify: 121 tjek, sim_mean 0.522, 15 suspekte frames (12%), 2 switch-risk (12.2s/55.4s). **Visuel verifikation af fundene: ÆGTE prod-trackingfejl fanget ved 24.8s (boksen står på tom baggrund/målstolpe — ingen spiller!) + korrekt flagget farlig 3-mands-crossover ved 11.9s.** 43s = delvis falsk alarm (rigtig spiller, ekstrem positur). Skygge-motorens fund-tidsstempler er klar til fremtidige P7/P22-evidens-gates.
- **VIGTIG LÆRING**: farve-features adskiller HOLD sikkert (hvid vs blå) men ALDRIG identiske holdkammerater — dér skal margin-regel give UNCERTAIN (fail-safe by design). Videoen er MONTAGE (flere scener/baner/lys) — profil fra taps dækker alle scener. yolov8n.onnx i models/ er stadig UGYLDIG (29 bytes) — ægte persondetektor (P3 fuld MOT) kræver valid model eller integration.
- **NÆSTE (efter validering på FLERE rigtige uploads, jf. briefens egen regel)**: P7 kobling til analysis-gates (shadow-data → evidens-gate), P8 marker-forbedringer fra verified track, P9 gap-bridging, P10 proof-udvidelse. AKTIVÉR INGEN gates før shadow-tal fra flere rapporter viser lav falsk-alarm-rate.
- Genbrugstest: `/app/backend/tests/run_cv_shadow_check.py {report_id}`. ⚠️ REQUIRES REDEPLOY for prod-effekt (shadow kører kun i preview indtil da).
...
- Verificeret via seed_iter78_fpl (anchors 15/58/92/120/160): kort viser 00:15 (før 01:32-mismatch), key-moment 02:00, chip + note OK. Seed-bruger slettet igen.
- ⚠️ REQUIRES REDEPLOY.
- ÅBEN: bruger har ikke svaret på Resend-valg (kvitteringer i uønsket post — tracking fjernet fra receipts i part 7-opfølgning, men domæne-autentificeret afsendelse (Resend + DNS) er den rigtige løsning). Priserne står stadig på $1 i prod (brugerens test) — mindet ham om at sætte dem tilbage.

## Session (Jun 2026 — part 7) — DELT STRIPE-KONTO BUG: "1 Million Bolde"-kvittering + webhook registreret ✅ (self-tested: signerede webhook-events, guard + receipt OK)
- **PROD-BUG (rapporteret af bruger m. screenshot)**: Efter rigtige køb på scoutmeplay.com ($1 testpriser) modtog han "Tak for dit køb hos 1 Million Bolde / 50 kr-md pakke"-kvittering. ROOT CAUSE: Stripe-kontoen deles ml. ScoutMePlay og 1millionbolde.dk (business name "SCOUTMEPLAY BY MK 1M BOLDE"); kontoen havde KUN én webhook: https://1millionbolde.dk/api/webhook/stripe (enabled, checkout.session.completed) → 1MB-appen reagerede på ScoutMePlay-betalinger og sendte SIN kvittering. Bruger bekræftede: 1MB er AKTIVT → webhook må IKKE deaktiveres; 1MB-appens kode skal selv filtrere (instruks givet). Langsigtet anbefaling: separat Stripe-konto pr. projekt.
- **ScoutMePlay-webhook registreret via API** (bruger godkendte): we_1U2gmOPyHKLMizP3KQwLkkRD → https://scoutmeplay.com/api/webhook/stripe-embedded, events: checkout.session.completed + customer.subscription.created/updated/deleted + invoice.payment_succeeded/failed. Ny signing secret skrevet til backend/.env STRIPE_WEBHOOK_SECRET (aldrig printet). Indtil redeploy afviser prod events (gammel secret) — Stripe retrier i op til 3 dage; poll-path dækker kvitteringer imens (efter redeploy).
- **Fremmed-session guard i webhook** (server.py ~12799): hvis ingen txn OG metadata.brand != "ScoutMePlay" → return straks (ingen kreditering/e-mail for 1MB-betalinger). Alle vores sessions bærer allerede brand=ScoutMePlay via _SCOUTMEPLAY_METADATA. subscription/invoice-events no-op'er sikkert for fremmede subs (user-lookup fejler).
- Test m. korrekt HMAC-signerede events mod localhost: (1) fremmed 1MB-session → {"received":true}, INTET oprettet, INGEN e-mail; (2) ScoutMePlay guest_single txn → receipt sendt (buyer noreply@) + admin sale notification (scoutmeplay@gmail.com — ét internt testkøb "$1.00 · noreply@" i ejerens indbakke, kan ignoreres); beløb korrekt fra session amount_total ($1.00 — brugerens ændrede pris). Usigneret kald → 400 Invalid signature. Cleanup udført.
- 1MB-fix (brugerens ansvar i det andet projekt): 1millionbolde.dk-webhooken skal ignorere sessions den ikke selv har oprettet (slå session_id op i egen DB, eller kræve eget metadata-mærke) — ellers sender den fortsat forkerte kvitteringer for ScoutMePlay-køb.
- ⚠️ REQUIRES REDEPLOY (ny secret + guard + kvitterings-e-mails fra part 6 er stadig ikke i prod — bruger bekræftede "jeg har ikke deployed").

## Session (Jun 2026 — part 6) — PAYMENT RECEIPT EMAILS m. NEXT STEP ✅ (self-tested: 4/4 SMTP sends + idempotens + unpaid-regression)
- **Én idempotent kvitterings-pipeline**: ny `_send_receipt_email_for_session(session_id, session_obj)` i server.py (~12639, før stripe-embedded webhook) — atomisk claim (`receipt_email_sent` flag via find_one_and_update) så webhook OG status-polling aldrig dobbelt-sender. Kaldes fra 4 steder: stripe-embedded webhook (erstattede gammel inline e-mailblok), guest status (efter kreditering — dækker preview/uden webhook), subscribe status (paid), embedded status (paid). Beløb fra Stripe session amount_total (promo-koder afspejles) m. txn.amount fallback.
- **Kind-specifik "NEXT STEP"**: guest_single/prepay → "Upload your match video" → /upload; report_unlock → "Open your report" → /report/{id}; subscription/guest_subscription → dashboard + tier-note (2 el. 4 rapporter/md); scout_access → /players-database. Gæstekøb viser nu korrekt produktnavn ("Single Scout Report"/"ScoutMePlay Premium/VIP" — før: generisk "ScoutMePlay purchase" og forkert dashboard-CTA).
- `render_purchase_confirmation` (email_templates.py) udvidet m. optionelle `cta_label`/`cta_path`/`next_note` (bagudkompatibel) + NEXT STEP-sektion i branded chrome. Admin send-test panel-template opdateret m. eksempel. Admin sale-notification følger samme claim (ingen dobbelt-notifikationer ved webhook-retries).
- Test: direkte helper-kald m. fake paid txns → 4/4 rigtige SMTP-sends KUN til noreply@scoutmeplay.com (email_log: 2 receipt + 2 sale_notification, status sent), dobbelt-kald sendte IKKE igen, unpaid guest-status poll uændret, template-assertions OK. Alle test-txns + test-logs slettet.
- OBS: legacy hostede fallback-flows (kun når embedded-nøgler mangler) sender ikke receipt via status-poll — webhooken dækker dem i prod hvis konfigureret.
- ⚠️ REQUIRES REDEPLOY.

## Session (Jun 2026 — part 5) — BRANDED CHECKOUT PAGES /checkout/{single|premium|vip} ✅ (iter90: 100% backend 10/10 + frontend, 0 issues)
- **User**: "customize stripes pages... make them more real and professional... identical layouts for single/premium/vip, use my 3 uploaded layouts 100%". Valg: (1a) egne branded sider m. Stripe EMBEDDED checkout (eneste måde at matche layouts), (2a) samme checkout for gæster OG loggede brugere.
- **NY side `/app/frontend/src/pages/CheckoutPage.jsx`** (route `/checkout/:tier` i App.js): mørk branded venstre-kolonne pr. reference (kicker, Barlow-headline m. accent-underline, hero-billede m. overlay-subline, benefits, plan-summary-boks m. 7 checks, testimonial/VIP-upsell/promo, trust-row, footer-bånd "SECURE PAYMENT · POWERED BY STRIPE") + creme betalingskort til højre m. produkt-summary (navn/pris/chip/desc) og ÆGTE Stripe Embedded Checkout (Apple Pay/Link/amazon pay/kort — lokaliseret automatisk). Farver: single+premium lime #CCFF00, vip guld #F5C443. Hero-billeder genereret (Nano Banana) i `/app/frontend/public/checkout/hero-{tier}.jpg`. Mobil: stacked (hero → betalingskort → resten), 0 overflow @390; desktop: 2 kolonner m. sticky betalingskort. Testids: checkout-page/-back-btn/-kicker/-headline/-hero-img/-payment-card/-product-name/-price/-price-chip/-stripe-wrapper/-plan-summary/-benefit-{i}/-testimonial/-trust-row/-vip-upsell-link/-vip-promo/-footer-trust/-script-line/-loading/-error/-already-subscribed/-confirming-overlay.
- **Session-init pr. kontekst**: gæst → NY `POST /api/payments/guest/embedded` {tier} (kind guest_single/guest_subscription, ui_mode embedded, return_url /welcome?guest_session=); logget-ind single → /payments/embedded/prepay-upload; single?report={id} → /payments/embedded/unlock; premium/vip → NY `POST /api/payments/embedded/subscribe` (kind "subscription", return_url /dashboard?subscribe_session=; 409 hvis aktivt abo → venligt kort m. dashboard-link). onComplete: gæst → navigate /welcome?guest_session= (eksist. konto-oprettelses-flow); logget-ind → poll status → toast + navigate (/upload | /report/{id} | /dashboard). Fallback til hostet Stripe hvis embedded-nøgler mangler.
- **Backend (server.py)**: 2 nye endpoints efter get_guest_checkout_status (~11915); guest-status håndterer nu embedded guest_single via `_arm_real_stripe` (legacy proxy-branch bevaret); `embedded_prepay_upload` bruger nu `get_extra_report_price_for_user(user)` (abonnenter betaler deres extra-report-pris, price_tier i metadata) + prepay/unlock returnerer amount+currency.
- **Alle købs-CTA'er peger nu på /checkout/{tier}** (ikke checkout.stripe.com): DreamPricingTiers, PricingTiers, ReportPaywallTiers, StickyPricingCTA, DashboardPage UpgradeBanner (startSubscription + buyExtraReport), ReportPage handleUnlock (→ /checkout/single?report={id}). UploadPage's mid-flow prepay-modal BEHOLDT (embedded modal, ellers mistes upload-state).
- **GuestWelcomePage**: 404-session → øjeblikkeligt fejlkort (før: 37s polling).
- **iter90: 100%** — guest embedded alle 3 tiers m. korrekte priser, unpaid-status, 400/404/401, admin subscribe+prepay, alle testids/heroes/Stripe-iframe på mobil+desktop, CTA-navigation, /checkout/bogus → /, welcome-regression. 32 test-txns slettet. Genbrugbar pytest: `/app/backend/tests/test_iter90_embedded_checkout.py`. INGEN rigtige betalinger (LIVE Stripe-nøgler).
- OBS: `wait_until="networkidle"` timeouter på /checkout (Stripe-iframe) — brug domcontentloaded. Kendt præ-eksisterende: STRIPE_API_KEY=sk_test_emergent (proxy, bruges af legacy hostet single) vs live pk/sk — bevidst setup, ikke rørt.
- Stripe-produktnavne i iframen kommer fra Stripe-produkterne (fx "ScoutMePlay VIP Premium") — kan omdøbes i Stripe Dashboard hvis ønsket (ændrer IKKE priser).
- ⚠️ REQUIRES REDEPLOY.
...
- **`proofplayer.jsx`**: locked mode via `proof.locked` + `onUnlock`-prop; testids `proof-player-locked-note`, `proof-player-unlock-btn`; Lock-ikon i header.
- **`FreePreviewLanding.jsx`**: intro (localStorage `smp_cine_seen_{id}_fp` — SEPARAT nøgle så premium-introen stadig spiller efter køb) + sheet mounted øverst; replay-knap i header; `fpl-proof-open` (åbent kort, keyMomentT), `fpl-proof-keymoment`, `fpl-proof-locked-{0,1,2}`. Video: `${ASSET_BASE}${report.video_url}` (free preview HAR ægte video); hero-video pauses ved proof-åbning.
- **iter84: 100%** — count-up 84/100, volt end-line, show-once/replay, ægte video i sheet (scroll urørt), locked mode + unlock-scroll, Escape/close, 390px 0 overflow, sample-report regression (8.0 decimal, ingen /100, ikke-volt end line, demo-note, share-clip-knap). Temp-bruger+rapport slettet (genbrugslig seed: `/app/backend/tests/seed_iter84.py` seed|clean — SKAL inkludere full_report.scores-stub, ellers regenererer serveren teaser→null).
- VIGTIG adfærd: server regenererer teaser.overall_potential fra full_report.scores — free previews uden scores viser låst score, og introen viser da navn+moment+endline UDEN count-up (korrekt/ærligt).
- ⚠️ REQUIRES REDEPLOY.

## Session (Jun 2026 — part 2) — INTRO SHARE CLIP (MP4) ✅ (self-tested: generering + frames + auth-gates + E2E download)
- **`/app/backend/intro_clip.py`**: server-side render af cinematic-introen som delbar 1080×1920 story-MP4 (24fps, 8.7s, h264/yuv420p/faststart, LYDLØS — sociale platforme lægger selv musik på). Scener: mørk åbning m. pulserende "SCOUTMEPLAY PRESENTS" → bedste frame (pan) + navn + "MINUTE ts" + moment-titel → score-count-up 0→X (ease-out) + "OVERALL SCORE"/"YOUR STORY STARTS HERE" → endcard m. SCOUT[ME]PLAY-wordmark + scoutmeplay.com. Demo: chip "DEMO SAMPLE — PREMIUM REPORTS OPEN LIKE THIS" + "Sample player — get your own report" på endcard. Genbruger pdf_v2.derive_v2 (snapshotMoments) + _pick_hero_photo + fonts/. imageio_ffmpeg.write_frames m. macro_block_size=1; skriver .tmp → os.replace. Generering ~5s, caches i CARDS_DIR `{id}.intro.v{INTRO_CLIP_VERSION}.mp4` (bump version ved designændring).
- **Endpoints (server.py efter snapshot-card, ~10427)**: `GET /api/demo-report/intro-clip.mp4` (public, cache 1h) + `GET /api/reports/{id}/intro-clip.mp4` (owner/admin, 402 låst, 400 uden full_report). Verificeret: 200 video/mp4 · 401 uden auth · 404 ukendt · 400 uden full_report.
- **Frontend (PremiumReportV2 header)**: "Share intro clip"-knap (`intro-clip-share-btn`, Clapperboard-ikon) ved siden af "Play intro" — blob-fetch (timeout 120s), navigator.share m. fil ellers download + toast; "Creating clip…"-spinner + info-toast efter 2.5s ved første generering. Virker på både demo og premium.
- E2E verificeret: klik → ScoutMePlay_Intro.mp4 downloadet + success-toast; frames visuelt inspiceret (alle 4 scener flotte).
- ⚠️ REQUIRES REDEPLOY.

## Session (Jun 2026 — CINEMATIC INTRO + PROOF MINI-PLAYER ✅ (iter83: 100% frontend 18/18, 0 issues)
- **User-godkendt efter mockup-runde** (statiske mockups i `/app/backend/static/landing/mockup-{cinematic,rewind}{,-mobile}.html` — desktop + mobil, godkendt: 1a Ja, 2a Ja, 3b premium+sample m. demo-label).
- **Cinematic intro** (`report-v2/cinematic.jsx`): film-åbning ved FØRSTE åbning af rapport — mørk scene m. filmbjælker → spillerens bedste frame (snapshotMoments[0].thumb) + navn + "Minute {ts}. {moment title}." → score tæller op 0→X (rAF, ease-out) + "Your story starts here" → auto-fade til rapport (~8s). SKIP altid synlig; lyd FRA som default (toggle, WebAudio tick under count-up); prefers-reduced-motion springer helt over. Show-once: localStorage `smp_cine_seen_{id}` (real) / sessionStorage (demo). Replay: "Play intro"-knap i V2PageHeader (`cinematic-replay-btn`). Demo-badge: "Demo sample — every premium report opens like this" (`cinematic-sample-badge`). Body-scroll låst under intro.
- **Proof mini-player** (`report-v2/proofplayer.jsx`): ALLE onPlayAt-kald (action timeline, top strengths, pace, movement, score-meaning, parent metrics, grow-your-game, parents package) åbner nu bottom-sheet i stedet for scroll-hop til bundvideoen. Auto-seek til ts−6s + play (muted-fallback). Scroll-position URØRT (verificeret diff=0). Mobil: sheet over bund-tabs (bottom-[62px], z-85); desktop: flydende panel nederst til højre (430px). Demo: nærmeste taktiske frame (snapshot_moments FØRST i proofFrames — video_comments-frames i demo er placeholders!) + note "Demo sample — available in the premium report...". Luk: ✕ + Escape. Hovedvideo pauses ved åbning.
- **`sections.jsx` TopStrengthsCard**: NY mobil-pill "SEE THE PROOF · {ts}" (`v2-strength-proof-{i}`, md:hidden) — thumbnails var desktop-only, så mobil manglede helt play-adgang.
- **`PremiumReportV2.jsx` REWRITTEN** (fuld fil, overwrite): playAt = setProof (ikke scroll), cine-state, proofFrames, header replay. `App.css`: smp-proof-slide + smp-cine-* keyframes.
- **iter83: 100% (18/18)** — intro-faser, skip, sound-toggle, show-once, replay, sheet u. scroll-jump (diff=0), demo-note, close+Escape, desktop-position (x=1466 w=430), mobil-position (bottom=vh−62), 4 pills, 0 console errors, 0 overflow @390.
- **IKKE live-testet**: video-seek på RIGTIG premium-rapport (ingen rapport m. video+full_report i preview-DB — ejer ønsker den tom). Seek-logikken er identisk m. den gamle gennemtestede playAt (currentTime + play + muted-fallback). Verificér ved næste rigtige upload.
- OBS Playwright: click() auto-scroller til target — mål scroll-jump EFTER elementet er fuldt synligt.
- ⚠️ REQUIRES REDEPLOY.

## Session (Aug 8, 2026 — part 9) — EXIT-INTENT TILBUD + TEAMMATE REFERRAL ✅ (iter82: 100% — backend 15/15 + frontend E2E)
- **Exit-intent tilbud**: `ExitIntentOffer.jsx` (ExitIntentPopup + ExitIntentManager i LandingMinimal) — vises ÉN gang pr. session når besøgende har SET prissektionen (#pricing-section IntersectionObserver) og er på vej væk (desktop mouseout top / mobil hurtig scroll-op >450px). Countdown mm:ss, claim → POST /api/exit-offer/claim → localStorage smp_exit_offer → attach efter login/signup → rabat auto-anvendes ved checkout. Admin (Growth-fanen, `ExitOfferAdmin.jsx`): enabled/%, countdown-min, valid_hours, headline + **Preview popup** (ægte popup m. preview-flag) + stats (claims/attached). Endpoints: GET /exit-offer/config (public), POST /exit-offer/claim (public), POST /exit-offer/attach (auth), GET+POST /admin/exit-offer. Config i db.settings key=exit_offer_config. **Default: DISABLED** — admin skal slå til.
- **Teammate referral**: personligt link `{origin}/?ref={code}` (frontend bygger fra window.location.origin — virker i preview OG prod). App.js gemmer ?ref → localStorage smp_ref; auth-context `redeemGrowthCodes()` efter login/signup redeemer referral + exit-offer (én chance, fjernes uanset udfald). POST /referral/redeem: kun nye konti (<7 dage), ingen betalte rapporter, ikke selv-referral, én gang — BEGGE parter får referral_credit {percent, expires_at}. Dashboard: `ReferralInviteCard.jsx` (kopiér/del, credit-badge, inviterede-liste). Admin (Growth): `ReferralAdmin.jsx` — enabled/%, valid_days + fuld hvem-inviterede-hvem-tabel (GET /admin/referrals). Default: enabled 15% / 30 dage.
- **`growth.py get_active_discount` REWORKED**: samler kandidater (rapport-niveau 48h + bruger exit_offer/referral_credit + kampagne) og returnerer HØJESTE % — alle checkout-paths uændrede (de kalder allerede denne). BEVIDST valg: credits er udløbsbaserede (ingen burn-on-use) — gælder alle køb i vinduet; simplere end at røre payment-webhooks.
- **`/app/memory/FEATURES.md` OPRETTET** — komplet feature-inventar + "IKKE bygget endnu"-liste så forslag aldrig gentages. HOLD DEN OPDATERET.
- **iter82**: 100% (15/15 backend pytest + fuld frontend E2E inkl. popup-trigger, once-per-session, ?ref-capture, auto-redeem, admin-preview). DB gendannet (exit offer disabled, referral defaults, testdata slettet). Genbrugbar pytest: tests/test_iter82_growth_exit_referral.py.
- Kendte accepterede punkter: /exit-offer/claim er unauth (let at floode — evt. rate-limit senere); ingen credit-consumption efter køb (udløb styrer); VIP-rabat via referral kræver Stripe coupons (ikke bygget).
- ⚠️ REQUIRES REDEPLOY.
...
- **Landing swipe (mobil)**: `LandingSections.jsx` — HowItWorks (3 steps) + WhatsInside (4 features) er nu horisontale snap-carousels på mobil (w-[82%]/[85%] kort m. peek, scroll-dots via `useCarousel`+`CarouselDots`, testids how-steps-carousel/-dots, whats-inside-carousel/-dots). Desktop-grids uændrede. VIGTIGT: brug `w-[..%]` ikke kun `min-w` på flex-kort (intrinsic max-content gjorde kort 761px brede — fixet + verificeret 280px @390vw, scrollWidth 390).
- **iter80**: backend 13/13 (401/403/402/404/200-flows, PNG-validering), frontend share-flows + carousels + desktop-regression pass. Genbrugbar pytest: `/app/backend/tests/test_iter80_snapshot_cards.py`. OBS: signup er rate-limited pr. netværk → test-seeds skal indsætte bruger direkte i DB + login. Gamle seeds m. `unlocked:True` låser IKKE nye endpoints op — kræver `is_paid`/`manually_unlocked`.
- ⚠️ REQUIRES REDEPLOY.

## Session (Aug 8, 2026 — part 6) — SNAPSHOT-SEKTION REDESIGN v2 ✅ (iter78+iter79 100%, alle 4 flader: web sample/premium/free preview + PDF)
- **User**: erstat Snapshot med NYT reference-layout — 100% identisk i Demo, Free Preview, Premium OG PDF. Layout: creme sektions-kort "SNAPSHOT" ("The key moments we found in your match."), 2×2 ens kort m. farvet header-bar (ikon + kategori | timestamp): BIGGEST STRENGTH (mørkegrøn), SCOUT NOTICED (mørkegrøn), HIDDEN TALENT (orange/brun), BIGGEST DEVELOPMENT AREA (mørkerød) → foto m. taktisk annotering → titel + beskrivelse → "OVERALL PROGRESS"-strip (grøn label + trend-pil + progressNote + "Keep working and enjoying the game."). "Next Milestone" fjernet.
- **`report-v2/snapshots.jsx`** (rewritten v2): `SnapshotsSection` (2×2 SnapCard), `SnapshotAnnot` (path=grøn pil+stiplet ellipse, scan=hvid stiplet pil, run=orange stiplet kurve+målcirkel, circle/space=rød stiplet ellipse; dash-arrays komprimeret pga. preserveAspectRatio-stretch), `SnapFrame` m. pitch-fallback (aldrig opdigtede fotos), `SNAP_CARD_META` m. header-gradients. Desc skjules hvis == titel.
- **`derive.js`**: `snapshotMoments` — titel foretrækker snapshot-frasen (biggestStrength/hiddenTalent/developmentArea); noticed bruger event-titel medmindre `snapshot.scout_discovery` findes; desc dedup mod titel; eksplicit override via `full_report.snapshot_moments`. `nextMilestone` fjernet.
- **`PremiumReportV2.jsx`**: SnapshotsSection fuld bredde (Row 2), MatchStats+AgeComparison i 2-kol række nedenunder.
- **`FreePreviewLanding.jsx`**: fpl-snapshots i SAMME kort-design — 1 ægte åbent kort (grøn header-bar + ægte poster + strengths[0] + ægte anchor-ts) + 3 låste (farvet header-bar m. lås, blurred poster, "Unlocks with the full report") + "1 of 4 unlocked"-pill + unlock-knap → #scout-packages. Intet premium-læk.
- **PDF (`pdf_v2.py`)**: Python-port af snapshotMoments i derive_v2; NY dedikeret **side 2 = SNAPSHOT** (`_snapshot_page`: header m. kamera-chip, 2×2 `_snapshot_moment_card` m. linearGradient header-bars + `_snap_annot` reportlab-annoteringer + `_snapshot_progress_strip`); side 1 row2 = match stats + age comparison 2-kol; gammel `_snapshot_card` slettet; total_pages base 4→5 + alle footer-numre reindekseret. **`server.py` PDF_RENDER_VERSION 24→25** (invaliderer cachede PDF'er). Testværktøj: `/app/backend/tests/render_snapshot_pdf.py` (fitz-render af side 1-2).
- **`seed_demo_report.py`**: DEMO_VERSION=3 + snapshot_moments-titler matcher referencen ("One-v-one dribbling that produces goals" 00:58 / "Great scan before the pass" 01:28 / "Back-post runs timed like an older player" 03:59 / "Left foot as a real option" 02:40). Re-seedet i preview; prod auto-opdaterer ved redeploy (v3-bump).
- **iter79 (testing agent): 100% backend + frontend, 0 issues.** Verificeret: sample web (alle kort/labels/ts/billeder + gamle glance/quote/milestone fjernet), premium web real-mode (frames + pitch-fallback), free preview (design + ingen læk + unlock-scroll), PDF via API (200, 7 sider, side 2 = SNAPSHOT, sekventielle footers, alle senere sektioner intakte), 390px ingen overflow, ingen console errors. Kendt kosmetik: hidden-kort kan mangle ts når ingen timeline-match (bevidst — ingen opfundne timestamps).
- ⚠️ REQUIRES REDEPLOY.
...
- **Fix 2 — premium-opgradering (user: høje scores + detaljerede timestamps der sælger)**: scores 7/6/7/8→**8/7/8/9 (overall 8)**, dribbling+1v1+acceleration+confidence+response = 9, teaser 70→**84**, action_timeline 8→**13 events** m. detaljerede beskrivelser inkl. **GOAL (03:59)** + **ASSIST (04:10)**, match_stats 31 actions/7 dribbles/9-12 duels, scout_outlook: Pro Academy (4 dots) → Elite Pathway Potential (5 dots), "High — Ready to Be Tested", benchmark-tier pro_academy, beriget executive summary/parent summary/message_to_player m. konkrete tal (7/9 duels). Ærlige udviklingsområder bevaret (weak_foot 6, 2 neutrale events).
- **VERIFICERET**: slettede demo-doc + billeder fra uploads (= frisk prod) → genstart → auto-seed genskabte alt (log: "demo sample report seeded (v2)"); API: 13 events, scores 8/7/8/9, goal+assist=True, billeder 200; screenshot: fuld premium-render (8.0 gauge, Academy Level-badge, Top 20% bars, 9.0-kort).
- ⚠️ REQUIRES REDEPLOY — derefter virker /sample-report + alle links dertil på scoutmeplay.com automatisk.

## Session (Aug 8, 2026 — part 2) — STICKY CTA + SEO-PANEL + BEST-PRICE + SPILLER-STEMME ✅ (iter77 100%, 0 issues)
- **1. Sticky mobil-CTA (kun forside)**: `StickyPricingCTA.jsx` — fast knap i bunden (over MobileBottomTabs, bottom-[62px] md:bottom-0, lg:hidden) synlig via IntersectionObserver på `#pricing-section`; gæst → /signup?plan=single, logget ind → /upload. Tekst+on/off admin-editerbar: public `GET /api/settings/sticky-cta` + `PUT /api/admin/sticky-cta` (settings key `sticky_cta`), admin-kort `StickyCtaAdminCard.jsx` i Settings-fanen (testids admin-sticky-cta-*).
- **2. SEO-panel** (`seo_insights.py` + `SeoInsightsPanel.jsx` øverst i "SEO & Social"-fanen): organisk trafik fra first-party analytics (referrer matcher google/bing/ddg m.fl.) — sessions, andel, daglig graf, top-landing-sider; artikel-tabel (titel/keywords/views); LLM keyword-forslag `POST /api/admin/seo/keyword-suggestions` (gpt-5.4, JSON: keyword/audience/intent/interest/article_idea, caches i settings `seo_keyword_suggestions` — 8 stk. genereret og cachet). Ærligt labelet: interest = estimat, GSC kan kobles på senere for query-data.
- **3. Best-price på pris-kort** (`DreamPricingTiers.jsx`): live-beregnet fra admin-priser — Premium "2 reports/mo ≈ $X each — save Y%" (premium/2 vs single), VIP tilsvarende (vip/4); "BEST VALUE PER REPORT" guld-badge KUN på billigste pr. rapport (pt. VIP $12.50/90%). Opdaterer automatisk når priser ændres (læses fra /settings/price).
- **4. Spiller-stemme (a+b+c fra bruger)**: `growth._is_self(user_full_name, player_first)` → fornavns-match = kontohaver ER spilleren → spiller-stemme ("Your report is ready", "your game", "you could become") i ALLE 7 templates: report_ready, curve_reminder, conv_waiting/discount/discovery, abandoned (self_player-param, unit-testet begge stemmer). Call-sites opdateret i growth.py (conv+abandoned m. user-lookup) og server.py (report-ready + curve — FIXEDE også bug: brugte `u.name` som aldrig fandtes, nu `full_name`). Neutraliseret: welcome, activation, discount-campaign ("The full story behind every number"), pdf_v2 ("the selected player"), LandingMinimal FAQ, LandingSections ("Mark the player"), SampleReportShowcase ("your player"/"the player's game" i stedet for "your child"). `FreePreviewLanding.jsx`: `selfPlayer` const → 10 tekster skifter til du-form når navn matcher.
- iter77: backend 7/7 + alle frontend-flows 100%; SEO-tabel titel-overlap fixet efterfølgende (truncate + max-w) og screenshot-verificeret. Genbrugbar test: /app/backend/tests/test_iter77_sticky_seo.py.
- ⚠️ REQUIRES REDEPLOY. Guide-funnel emails (guide_emails.py) er BEVIDST forældre-stemme (forældre-guide-audience).
...
- **1. BLOG (root cause: `uploads/blog/` er git-ignoreret → covers deployes aldrig; artikler lå kun i preview-DB → prod-DB tom; Blog Studio lavede kun kladder uden cover)**:
  - NY `blog_seed.py` + `blog_seed_data.json` (6 launch-artikler eksporteret) + covers bundlet i git-tracked `static/blog_covers/`. Startup-hook i server.py: kopierer covers → uploads/blog hver boot; seeder de 6 artikler ÉN gang pr. database (settings-flag `blog_seed_v1_done`, kun hvis 0 published). Testet mod frisk scratch-DB: 6 indsat, idempotent.
  - `blog_studio.py` REWRITTEN: `_generate_cover()` — Gemini `gemini-3.1-flash-image-preview` (Nano Banana, emergentintegrations playbook) genererer fotorealistisk cover i samme stil (golden-hour grassroots, no text), PIL→JPEG 1600w, gemmes i uploads/blog + **flushes til R2** (`/api/media/blog/{fname}` = overlever redeploys). Cover-fejl blokerer ALDRIG artiklen. NY `publish_now` på GenerateRequest → artikel publiceres med det samme (default ON i UI); weekly auto-draft forbliver draft (nu med cover).
  - `BlogStudioPanel.jsx`: "Publish immediately" checkbox (testid `blog-studio-publish-now`) + copy om auto-cover.
  - E2E: rigtig generering → "What to Eat Before a Youth Football Match" publiceret live m. R2-cover (200, 258KB), vises på /blog. ARTIKLEN ER BEHOLDT (7 published i preview nu).
- **2. STRIPE PRIS-SYNC** (`PUT /api/admin/pricing` i server.py): når premium/vip ændres → opretter NYT Stripe recurring Price på samme Product, arkiverer gammelt (`active=false`), opdaterer settings-cache `stripe_subscription_{tier}` → nye subscriptions checkouter til nyt beløb MED DET SAMME. Eksisterende abonnenter beholder deres pris (Stripe-standard). Response: `stripe_synced`/`stripe_sync_error`. AdminPage: note + fejl-warning opdateret, toast viser sync. VERIFICERET mod RIGTIG Stripe (live key): 29.99→30.99 → nyt price aktivt $30.99/mo, gammelt arkiveret → gendannet 29.99. Single/extra-priser var allerede dynamiske (læses ved session-create). VIGTIGT til prod: efter redeploy skal ejeren trykke "Save plan prices" ÉN gang i prod-admin for at synce Stripes gamle beløb.
- **3. EMAIL KLIK-TRACKING**: `email_service._track_links()` omskriver alle href-links til `{SITE}/api/email/click/{log_id}?u=…`; `email_log.py` REWRITTEN: klik-endpoint (302 redirect, first-click-wins `clicked_at`+`clicked_url`, klik sætter også opened_at; open-redirect-beskyttelse: kun scoutmeplay.com/*.emergentagent.com/localhost, ellers → forside). Admin-log API + EmailAdmin UI: Clicked-kolonne + "X sent · Y opened · Z clicked (CTR)". Unit tests + curl: 302 korrekt/evil→fallback, clicked:1 i admin. Klik-tracking i rigtige indbakker virker EFTER redeploy (links peger på scoutmeplay.com).
- ⚠️ ALT REQUIRES REDEPLOY. Prod-noter: (a) brugerens eksisterende prod-kladder (uden cover) kan publiceres manuelt eller genereres igen, (b) growth_emails_enabled skal slås til i prod-admin, (c) tidligere sendte emails har ingen klik-links (kun nye).

## Session (Aug 7, 2026) — KONVERTERINGSPAKKE KOMPLET ✅ (iter76 100%, 0 issues + rigtige email-sends verificeret)
- **User-approved package**: aktiverings-emails (24h/72h efter signup uden upload), interaktiv demo-rapport, sløret preview-paywall, abandoned-checkout email, admin email-synlighed (hvem fik + hvem åbnede).
- **Emails (growth.py + email_templates.py)**: `activation_sweep` (24h + 72h nudges, flags på user-doc = max én gang, skipper admin/scout/@example.com, MAX 14 dage gamle konti; NY guard: 72h-mail kræver mail1 sendt ≥24h siden; NY fix: læser `full_name` ikke `name`). Abandoned checkout: initiated txns 1-48h gamle, skipper betalte, én gang. Alt kører i eksisterende `conversion_loop` (15 min) gated af settings `growth_emails_enabled` (= TRUE i preview). Admin `POST /api/admin/conversion-sweep?dry_run=` returnerer nu BÅDE `results` + `activation`.
- **Email-log + open tracking**: `email_service.py` logger hver send til `db.email_log` (id/to/subject/category/status/ts/opened_at) + indlejrer 1×1 pixel `{SITE}/api/email/open/{id}.png` (SITE = SITE_PUBLIC_URL/FRONTEND_URL env, fallback scoutmeplay.com — opens tracker først i PROD efter redeploy). NY `email_log.py` router: public pixel-endpoint + admin `GET /api/admin/email-log` (search/category/stats). `enable_email_log()` kaldt ved startup (sync pymongo — sends kører i executor-tråde).
- **EmailAdmin.jsx**: "Sent log — who received & opened what" sektion, DEFAULT ÅBEN (testids email-sent-log, email-log-search, email-log-stats, email-log-row-{id}), viser åbnings-% + ✓ tidsstempel + ærlig pixel-disclaimer.
- **Demo-rapport**: `seed_demo_report.py` (idempotent, fiktiv "Noah (Sample Player)", id=`demo-sample-report`, komplet FULL_REPORT-skema inkl. parents_package/missions/action_timeline/cross_verification; billeder uploads/demo-player.jpg + 2 frames). Public `GET /api/demo-report` (server.py L6798). NY `DemoReportPage.jsx` på `/sample-report` (sticky sample-banner + PremiumReportV2 + bund-CTA; testids demo-report-*). Landing hero secondary CTA → /sample-report. FIX: App.js manglede DemoReportPage-import (runtime error) — tilføjet.
- **Paywall**: FreePreviewLanding `fpl-locked-sections` (10 slørede sektions-rækker + unlock/sample knapper) — verificeret INGEN full_report-læk i API for ikke-betalende (include_full=False server-side).
- **E2E VERIFICERET**: 3 RIGTIGE emails sendt til scoutmeplay@gmail.com (activation 24h + 72h + abandoned checkout — ligger i ejerens inbox + email_log), dublet-beskyttelse bekræftet (gensend = tom), open-pixel → opened_at sat. iter76: 7/7 backend pytest + alle frontend-flows 100% (genbrugbar test: /app/backend/tests/test_iter76_demo_paywall.py).
- **VIGTIGE NOTER**: (1) Open-tracking i rigtige indbakker virker først EFTER REDEPLOY (pixel-URL peger på scoutmeplay.com). (2) SMTP "sent" ≠ garanteret leveret/læst — pixel undertæller (Apple Mail m.fl.). (3) Efter prod-deploy: eksisterende prod-brugere >14 dage gamle får ALDRIG aktiverings-mails (launch safety). (4) Growth-emails i PROD kræver at settings `growth_emails_enabled` sættes TRUE i prod-DB via admin.
- ⚠️ REQUIRES REDEPLOY.


## Session (Aug 5, 2026 — part 6) — HERO + IMAGE STRIP + PRICING BANNER FIXES ✅ (self-tested mobile screenshots)
- User: (1) hero & "Built for the next level" pictures keep but SMALLER on mobile, unreadable font colors, too stiff — need life/animation; (2) black background on pricing banner must go (mismatched).
- ROOT CAUSE of unreadable text: tailwind token `volt` = #2D6B3D (DARK GREEN, not lime!) — text-volt/bg-volt over dark photos was invisible. Fixed with explicit colors (gold #F5C443 / lime) in overlay spots; did NOT change the volt token (used app-wide on light bgs).
- `LandingMinimal.jsx` hero composition: wrapper now max-w-[430px] mx-auto on mobile + aspect-square (sm:4/5) = smaller; ken-burns on photo; "Pro scout · live" now dark glass chip w/ gold text; bottom scrim added under caption; corner brackets gold; floating report card has continuous float animation; stats chip now forest gradient w/ gold 7.8 square + gentle bob; scan line now travels 22%→72% with gold glow.
- `LandingSections.jsx` ImageStrip: mobile aspect square (was 5/6) = smaller; ken-burns; stronger scrim; eyebrow/lines/brackets gold; "next level." explicit gold w/ text-shadow; all three text blocks are framer-motion whileInView (once:false → replays on scroll) staggered reveals.
- `DreamPricingTiers.jsx`: BLACK wrapper (#070A07) → warm cream gradient w/ forest border+shadow; TrustStrip restyled for light bg (forest/gold icons, ink text); swipe arrows forest bg w/ gold chevrons + gold pulse; swipe-hint text ink. NOTE: this component is shared (landing, free preview, dashboard) — consistent everywhere.
- ⚠️ Incident: parallel search_replace on DreamPricingTiers corrupted file tail (duplicate fragment) — cleaned up; avoid many parallel edits on one file.
- Verified: 3 mobile screenshots (hero, strip, pricing) all correct. ⚠️ REQUIRES REDEPLOY.

## Session (Aug 5, 2026 — part 5) — SAMPLE CARDS v3: LESS MATTE GREEN/NEON, PER-SWIPE SURPRISE ANIMATIONS ✅ (self-tested mobile screenshots)
- User feedback on part 4: "for meget kedelig grøn mat baggrund og for meget neon grøn… mere fade baggrund… flere fede animationer… hvert kort skal være specielt og tilbyde overraskelse når man swiper".
- v3 rewrite of `SampleReportShowcase.jsx`: (1) NO flat matte green — every dark card now has photo bg (dossier=carousel-bg-7 kneeling, superpowers=bg-6 rain + ghost '3' watermark, plan=bg-4 tunnel, pace=boots, curiosity=golden hour, road=silhouette, cta=stadium) or texture (pillars=pitch-lines SVG watermark, parents=giant quote mark, library=dot-grid); ken-burns slow zoom on photos. (2) NEON REDUCED: gold (#F5C443) is now the main accent (kickers, icons, timeline, radar, arrows, bar end-dots); lime kept ONLY for 8.2 score + CTA button. (3) SURPRISE PER SWIPE: Card shell has IntersectionObserver (threshold .55) toggling `.smp-inview` → all `.smp-a-*` reveal animations REPLAY every time a card swipes into view. Per-card signatures: dossier 8.2 counts up 0→8.2 (MutationObserver-driven), pillar bars fill + gold dots pop, superpowers rows alternate slide L/R + golden key tilts in, pace sprint path DRAWS (pathLength dashoffset) + chips pop, curiosity locks wiggle, parents rows alternate slides, plan timeline grows downward + nodes cascade, road steps light sequentially, scout-reply bubbles arrive like live chat (0.55s/1.4s) + typing dots, library radar pops + notification slides up, CTA button slides up w/ shine.
- Verified: 11 cards render, animations completed in screenshots at 3 scroll positions (mobile 430px). Scroller/arrows/dots/testids unchanged. ⚠️ REQUIRES REDEPLOY.

## Session (Aug 5, 2026 — part 4) — SAMPLE REPORT CARDS: FULL DESIGN-AGENT REDESIGN ✅ (superseded by part 5)
- User rejected part-3 version ("stadig uinspirerende, flade, statiske… et kort helt sort i baggrunden er grimt… mere levende, mere indbydende"). Called design_agent → blueprint saved at /app/design_guidelines.json (rules: NEVER pure black backgrounds, green gradient photo overlays, glassmorphism bg-white/10 backdrop-blur, layered depth/noise, glowing live elements).
- `SampleReportShowcase.jsx` REWRITTEN again per blueprint (same size/scroller/testids): 1 Dossier (forest gradient, grain, glass name pill w/ live dot, 86px glowing 8.2, gold quote, glass trust box) · 2 Pillars (light, lime-gradient bars w/ glowing end dots) · 3 Superpowers (forest-light, glass rows, tilted gold "golden key" card that straightens on hover) · 4 Pace (PHOTO muddy boots carousel-bg-2 + green gradient overlay + animated dashed sprint SVG path + glass stat chips) · 5 Curiosity (PHOTO golden-hour parent+child cover-bounce-back.jpg + glass locked questions + pulsing "Unlock the full picture") · 6 Parents (light Q→A) · 7 Plan (dark vertical dashed lime timeline, glass nodes) · 8 Road Ahead (PHOTO dusk silhouette + green/gold overlay — NO black — "From the local pitch to the scout's notebook") · 9 Scout Reply (light chat bubbles, avatar w/ green online dot, "scout is typing…" animated dots) · 10 Library (forest gradient RADAR animation w/ sweeping cone + pulsing dot, "You've been spotted", "A scout opened his profile today · 3× this month · while he was at training") · 11 CTA (stadium photo, forest-tinted gradient, "It starts with one video").
- All photos are OUR generated realistic assets (user's preference) — no stock. Hover: photos scale-105. Fixes after screenshots: Road step rows two-line layout, Library text no truncation.
- Verified: 11 cards, desktop + mobile screenshots approved; scroller/arrow/dots logic untouched. ⚠️ REQUIRES REDEPLOY.

## Session (Aug 5, 2026 — part 3) — SAMPLE REPORT SHOWCASE REDESIGN ✅ (superseded by part 4)
- User: keep same card size, but slides must sell dream/curiosity/learning/discovery + add 4 new cards (dreams/road, real scout reply, seen in library, parent curiosity).
- `SampleReportShowcase.jsx` REWRITTEN (same shell 380px/minHeight 440, same scroller/arrows/dots/testids): now 10 content cards + CTA = 11 pages with page counters (x/11) — "leafing through a report" feel.
- Deck order: 1 Dossier (glow on 8.2) · 2 "Things He Never Knew" (animated bars) · 3 "Superpowers He Didn't Know He Had" · 4 "How Fast Is He Really?" · 5 NEW "What You'll Discover" (questions w/ BLURRED locked answers = curiosity gap) · 6 What Parents Ask · 7 "A Map to Who He's Becoming" · 8 NEW "The Road Ahead" (dark cinematic w/ carousel-bg-1 silhouette, glowing path Today→This season→Next season→When he's ready, honest "no promises" footer) · 9 NEW "A Real Scout Replied" (chat bubbles from professional scout, timestamped moment, personal drill) · 10 NEW "Found in the Scout Library" (search bar + profile row "Viewed by scouts · opened 3 times this month · while he was training", consent footer) · 11 CTA (unchanged).
- New CSS anims: smp-fill (bar grow), smp-rise (staggered entrance), smp-scoreglow. Brand rules respected (no "AI"/"percentile", scout used only for real scout features, honest dream-selling).
- Verified: 11 cards render, swipe/arrows/dots work, mobile + desktop screenshots approved by agent. Not run through testing_agent (single-component UI change, scroller logic untouched). ⚠️ REQUIRES REDEPLOY.
...
## 2026-08-11 — Numbered category layout (01–10) for Premium mobile + Free Preview (built + testing-agent 100%, iteration_91)
User approved the category mockups (mockup-premium-categories.html / mockup-free-categories.html) and chose: build into real reports, all collapsed by default (only 01 Player Snapshot visible), single-open accordion.
### Frontend
- NEW `/app/frontend/src/components/report-v2/categories.jsx` — `PremiumMobileCategories`: sticky dark tab bar (ANALYSIS/EVIDENCE/DEVELOPMENT/PARENTS/VERDICT, jumps + opens target category; top offset measured live from main-nav or demo banner height), 01 SnapshotHero card (photo, gauge, quad facts, expandable scout first impression, verified line, play-intro/share-clip buttons), CatRow single-open accordions 02–10 with score chips (bucket avg from full_report technical/tactical/physical/mentality; IQ = mentality + scanning/decision_making/game_understanding), count chips (impact/evidence/development), heart chip (parents). Bodies reuse ALL existing components (SkillMeaningCard filtered per bucket, Movement/Pace, MatchStats/AgeComparison/Progress, Snapshots/TopStrengths/ActionTimeline/VideoHighlight, DevPriorities/Roadmap/TrainingPlan/Missions/GrowYourGame/DreamPath, ParentTips/Metrics/Corner/Package, CoachNotes/ScoutOutlook/Discovery). Sticky bottom PDF bar (pm-pdf-bar) above the 62px mobile tab bar. Empty categories are hidden and numbering re-flows.
- `PremiumReportV2.jsx` — isMobile matchMedia(<1024px) branch: mobile → PremiumMobileCategories; desktop → ORIGINAL grid untouched. New props onDownloadPdf/downloadingPdf (passed from ReportPage line ~2144). Cinematic intro + proof sheet shared by both branches.
- `FreePreviewLanding.jsx` — fully restructured per approved mockup: compact header → 01 snapshot card (FreeGauge: open potential /100 when teaser exists, else blurred+locked overall) → YOUR FREE PROOF full-width card (BIGGEST STRENGTH tag, 1 OF 4 OPEN, ▶ first-anchor timestamp → proof sheet) → YOUR FULL ANALYSIS 9 locked rows 02–10 (02 blurred chip; evidence row opens locked proof teaser; others scroll to packages) → discount banner → ScoreMeaningTeaser → DreamPathTeaser → unlock CTA → DreamPricingTiers → trust bar. Sticky mobile unlock bar (fpl-sticky-cta, above bottom tabs); gift pill moved to bottom-[134px] mobile. REMOVED old clutter: hero video block, "what we discovered", preview-insights grid, 4-card snapshot teaser, key moment + missing list, locked sections list, 10% ring.
### Tested (iteration_91 — 100%, 0 issues)
- Premium mobile 390px (/sample-report): categories render, single-open verified, tabs jump+open, proof sheet, chips, no overflow, no console errors. Desktop 1440px: original grid, mobile layout NOT in DOM. Free preview 390px (seeded report): snapshot 78 POTENTIAL, proof 00:26 → sheet "MOMENT AT 00:26 / playing from 00:22" (frame-sync rule intact), 9 locked rows, scroll-to-packages, ShareUnlockModal from gift pill, sticky CTA above tab bar. PDF bar on seeded paid report: sticks above tabs, GET /api/reports/{id}/pdf → 200 (2.5MB). All seeded test data deleted after test.
### Backlog (non-blocking, from review)
- pdf_v2.py derive line ~350 fragile if parent_summary is a string (only artificial seeds; real reports use dict).
- User has NOT yet visually confirmed the live implementation on their own phone (mockups were approved; implementation preview-shipped + agent-tested).

## 2026-08-11 (2) — Upload-flow bugfixes (user-reported with 3 screenshots)
1. **"Generation was interrupted" at end of premium analysis (CRITICAL, root-caused + fixed + verified)**: Session 124 had tightened the Gemini timeout in `call_gemini_with_video` to 150s httpx / 180s outer for ALL calls — but the FULL-report dossier prompt over a real match video takes 3–7 min, so every full report timed out (3 failed attempts in logs for report f5748eb6). Fix: `timeout_s` param (default 150 for gate/preview unchanged); full-report, full-retry and cross-verify calls now pass `timeout_s=420`. User's real report regenerated successfully after fix (full_report_status=ready, renders in mobile category layout with real scores 7.3/6.0/7.3, verified line "6 moments re-checked", PDF bar).
2. **"Can't find variable: EmptyRanges" red overlay during analysis**: WebKit-INTERNAL bug — Safari's own built-in <video controls> JS (played/syncControl/handleEvent stack), not our code. Suppressed via window error/unhandledrejection guards in `src/index.js` + webpack dev-overlay `runtimeErrors` filter in `craco.config.js`.
3. **"AbortError: The operation was aborted" overlay on free preview**: benign cancelled media play()/load / dismissed share sheet. Same suppression (guards ignore AbortError, "operation was aborted", "interrupted by a call to pause", ResizeObserver loop noise).
Note: red overlays only ever appear in Preview (webpack dev overlay) — production builds never show them, but the underlying noise is now swallowed everywhere. Files: `/app/backend/server.py` (call_gemini_with_video + 3 call sites), `/app/frontend/src/index.js`, `/app/frontend/craco.config.js`. USER MUST DEPLOY for production effect.

## 2026-08-11 (3) — Report quality fixes (user-reported, 3 issues, all verified in Preview)
1. **Empty snapshot pictures**: the snapshot card text-matcher (derive.js `buildSnapMoment` + identical logic in pdf_v2.py `_sm_moment`) found no timeline event for 3/4 cards (only 3 events, brittle 4-char token match) and had NO frame fallback — cards rendered placeholder graphics despite 4 verified frames existing. Fix: frame-first fallback `takeAnyFrame()`/`_take_any_frame()` — every card now shows a real verified frame; displayed timestamp AND caption come from that exact frame's video_comment (same-moment rule preserved). Verified: 4/4 cards with frames on web (00:03/00:48/00:19/00:33) AND in PDF derive; PDF grew 2.5MB→11.7MB (frames embedded).
...
## 2026-08-12 (3) — New front-page hero "Turn your game into opportunity" (verified in Preview)
- User provided reference screenshot; rebuilt the HeroSection in `/app/frontend/src/pages/LandingMinimal.jsx` (the ACTIVE minimal landing variant) to match: headline "TURN YOUR GAME INTO OPPORTUNITY." (green accent + hand-drawn brush SVG), new subtext ("Upload your football video and get a detailed ScoutMe Pro analysis…"), 3-feature icon row (Analyse your game / Get seen by scouts / Find trials & opportunities, Crosshair/Eye/ShieldCheck with dividers), full-width green-gradient UPLOAD YOUR VIDEO button (upload icon + divider + football icon, glow/shine animations kept), two cream secondary buttons (How it works / See a sample report), trust line + sign-in kept, live ticker below kept.
...
## 2026-08-12 (12) — Quality Engine round 2 (self-tested: curl end-to-end + screenshots)
1. **Documentary cover generation**: QC dialog shows "Generate documentary cover" when cover judged TOO_GENERIC/REVIEW or missing (NONE) or recommendation REPLACE/GENERATE. Backend: POST /api/admin/quality/blog/{id}/generate-cover (background; gpt-5.4 brief → gemini image in ad-studio PHOTO_STYLE, 16:9, saved to uploads/blog/qc-cover-*.jpg + alt text; stored as proposed_cover in quality_checks). Dialog: CURRENT vs PROPOSED side-by-side + Approve & Apply (writes cover_image_url+alt via whitelist, now incl. cover_image_url) / Generate again / Reject. Verified: generation + apply + revert.
...
## 2026-08-12 (14) — Blog fix-button + Trial cover godkendt + ANTI-GENERIC IMAGE FORMULA (self-tested end-to-end)
1. **Blog "Fix red marks"**: POST /api/admin/quality/blog/{id}/fix → background _fix_blog: gpt-5.4 corrects ONLY flagged text issues in content_md (keeps structure/length/language, 50%-length safety guard) + applies stored proposed meta fields, then auto re-runs QC. Dialog button now shows for blog too (kind-aware label). Verified: trial article fixed (content_md + cover_image_alt) → PASSED 98/100, article intact (8313 chars, 20 headings).
2. **Trial-cover**: documentary cover live on 'How to Prepare for Football Trials' (qc-cover-5f798fc1-d7be2e.jpg); alt corrected by fix to match actual photo.
3. **IMAGE FORMULA rolled out**: ad_studio.PHOTO_STYLE rewritten as the full ScoutMePlay anti-AI formula (real moment, human imperfection, imperfect composition, camera language 35/50/85-135mm, natural light, football correctness, face-optional, natural colour — never green-tinted, full forbidden list). Used by ALL generation: Ad Studio images, ad regen, blog covers. New exports: VARIETY_CONCEPTS (8 rotating categories: DETAIL/ACTION/OBSERVATION/PREPARATION/HUMAN/AFTERMATH/ENVIRONMENT/POV — random rotation in cover briefs) + IMAGE_QC_RUBRIC (golden rule judge criteria) appended to BOTH QC system prompts (ad_studio QC_SYSTEM + quality_engine QC_SYSTEM).
4. **5 formula scores**: blog QC now returns image_scores {authenticity, football_realism, emotional_relevance, originality, brand_fit} 0-100; displayed as chips in dialog (qc-image-scores). Verified: generated cover scored 100/100/100/90/100 AUTHENTIC.
- REGRESSION FOUND+FIXED: the sentence-boundary body truncation in _check_blog had been lost in an earlier edit (raw [:5000] cut caused false 'grammar' flags quoting mid-word text). Re-applied with trunc note. LESSON: after fixing file-corruption/syntax issues, re-verify adjacent recent edits survived.

## 2026-08-12 (15) — Landing photos formula check (self-tested end-to-end + UI screenshot)
- New Quality-tab section "Landing photos — formula check" (quality-landing-check button): scores all 16 in-use hero/landing/report-bg photos on the 5 formula scores in chunked vision calls, plus dhash repetition among the set. Backend: LANDING_PHOTOS registry + _check_landing() + POST /api/admin/quality/landing/check (kind=landing/site).
- RESULTS (overall 62/100, brutally honest): KEEP: hero-player.jpg day hero 88 AUTHENTIC (user-approved hero validated), all 5 carousel-bg 94-99, bg-archetype-aerial 87. FAILING (TOO_GENERIC/REPLACE): finalcta-stadium.jpg (3!), auth-hero-player.jpg (5, fake shirt branding + cutout composite), hero-free.jpg (7), hero-premium.jpg (11, photoshopped composite), hero-player-night.jpg (13 — NOTE: night hero is a user-approved feature w/ moon+mist; judge flags fake beams/grading — USER decides, do not replace without approval), footer-hero-turf (56), bg-benchmark-tunnel (68), bg-scout-hero (81 purpose mismatch), radar-hero-stadium (85 IMPROVE).
- No auto-replacement built by design — report only; replacements need user approval per image.
- NEXT obvious step (not built): "Generate documentary replacement" button per failing landing photo (like blog cover flow) with CURRENT vs PROPOSED approval, careful with hero-night (user-approved feature).

## 2026-08-12 (16) — Landing photo replacement + new night hero proposal (self-tested end-to-end)
1. **"Generate documentary replacement"** per failing landing photo (verdict != KEEP) in Quality tab: POST /api/admin/quality/landing/{fname}/generate (background; gen_hint per photo in LANDING_PHOTOS registry + failing issues as negative constraints + PHOTO_STYLE formula; saved as static/landing/qc-new-*.jpg; stored in landing doc gen.{sanitized} map). CURRENT vs PROPOSED side-by-side → Approve & Replace / Generate again / Reject. POST .../apply backs up original to static/landing/backups/{fname}.{stamp}.bak then overwrites the live file (site updates without code changes).
2. **Verified**: apply changed the file (md5 diff), backup created, restore-from-backup returns exact original. Test apply was REVERTED — nothing on the site changed; owner decides.
3. **Pending proposals awaiting owner approval in Quality tab**: (a) hero-player-night.jpg — new documentary night hero: lone player from behind, ball at feet, REAL distant floodlights with natural pools (no beams), dark navy sky with space for the moon overlay, natural ground mist — layout-compatible with existing hero composition/animations. (b) finalcta-stadium.jpg — grounded golden-hour grass-roots pitch, worn goal, no hype.
4. Duplicates scanner now ignores qc-new-* proposal files.
- NOTE: hero night moon/mist overlays are UI layers (LandingMinimal) — they render on top of whichever night image is applied, so approving the new night hero keeps moon+mist features working.

## 2026-08-13 (17) — Telestration Level 1: AI-gated ring on non-anchor frames (self-tested, positive + negative cases)
- DISCOVERY: a safe telestration pipeline already existed (server.py _telestrate_verified_frames): anchor-locked frames get spotlight+ring+name chip via tap-anchor template/blob refinement (live: 4/5 frames ringed in report f5748eb6). AI-boxes were previously deemed unsafe → unused.
- BUILT: safe extension for identity-verified frames WITHOUT a tap anchor — telestration.find_player_double_gated(): (1) Gemini detect on full frame, BOTH coordinate-order interpretations returned as candidates ([y,x,y,x] asked but models sometimes answer [x,y,x,y] — proven live), (2) GPT-4o gate 1 verifies the padded region crop contains the tapped player, (3) zoomed Gemini re-detection inside the confirmed region (much more accurate), (4) GPT-4o gate 2 verifies the tight box, (5) kit-colour blob refine for feet (used only if center inside the confirmed box). ANY doubt → None → no graphics. Wired into the second pass of _telestrate_verified_frames (runs automatically post-full-report for new Premium reports).
- VALIDATED: negative cases — hallucinated tree box gated; wrong blue-kit blob gated; small/edge player honestly gated ("not enough distinctive information"). Positive case — player #15 found, double-confirmed via shirt number, rendered spotlight + ring + 'ALMIN · TRACKED' chip correctly.
- Cost: ~2-5 LLM calls per non-anchor frame (typically 0-2 such frames/report). Frames render in place → Premium report v2 + PDF + Free proof all benefit automatically.
- LESSON logged: Gemini box_2d coordinate order is unreliable — always disambiguate both interpretations and gate with a second model.
- Frame_02 in the test report remains honestly un-ringed (gates refused — by design, never a wrong ring).

## 2026-08-13 (18) — Telestration Level 2: TRACKED PROOF CLIPS (tested, iteration_94: 100%)
- User request: "Ring I Proof-Klip: Byg Niveau 2 — ringen følger spilleren gennem de korte videobeviser". User choices: (a) only clips with high coverage (>=90%) ship, rest fall back to normal video; (b) manual/admin backfill only for existing reports (new premium reports get clips automatically via _persist_video_frames); (c) ring UNDER the feet with a soft fade so it never disturbs the clip.
- ARCHITECTURE (final, after 3 failed tracker iterations): ring positions come STRAIGHT from the report's ground-truth `player_track` points (tap-seeded, colour-vetoed, drift-guarded, 12.5Hz) — NO re-tracking. `/app/backend/tele_clip.py`: plan_window() picks the contiguous dense track run around frame_picked_ts (exact tap time, NOT the rounded mm:ss timestamp — that 0.6s offset was the original wrong-seed bug), interpolates/smooths, renders subtle ground ellipse (dark under-stroke + lime, alpha<=0.85, 0.35s fade in/out, chip fades with ring) via imageio libx264 + `-movflags +faststart` (moov at end made browsers fail → REQUIRED).
- SAFETY GATE (server.py _teleclip_verified_window): clip edges >0.7s from the tap are cropped WITH the ring drawn in and verified by GPT-4o `verify_ring_placement` (identity_verify.py) — "is the ellipse under THIS player's feet?" Unconfirmed edge shrinks 0.6s toward the tap, window <1.4s → NO clip. Caught real failures live: ring on empty grass (camera pan) and track drifted onto #13 instead of #15 — both honestly gated.
- Persistence: clips upload to R2 (`reports/{id}/frames/proofclip_{i}.mp4`), stale tele_clip_url cleared for ALL comments up front. Serving: signed media URLs (minor protection, `_sign_media_url` applied to tele_clip_url in `_serialize_report`). Frontend already wired: PremiumReportV2 playAt → ProofPlayerSheet uses clip when present (`proof-clip-video`, badge 'Player tracked'), onError/absent → graceful fallback to full video at the moment.
- Live result on report f5748eb6 (Almin #15): proofclip_1 (00:19, 3.3s) + proofclip_4 (00:54, 1.5s) shipped and visually verified frame-by-frame; 00:03/00:33/00:48 honestly gated. Coverage field stored as tele_clip_coverage.
- LESSONS: (1) template matching w/o global-motion compensation rings empty grass during pans — even conf 0.8; (2) tracker conf does NOT indicate placement quality at segment edges — only a different-model placement gate does; (3) "player visible in frame" is too weak a question — must verify RING PLACEMENT with ring drawn in; (4) Playwright Chromium has NO H.264 — clip playback falls back in automated tests, EXPECTED, not a bug; (5) imageio needs `-movflags +faststart` for browser streaming; (6) imageio==2.37.4 now pinned in requirements.txt.
- Reusable pytest: /app/backend/tests/test_iter94_tele_clips.py. Testing agent iteration_94: 100% backend + frontend, no issues.
- Backlog ideas (not built): admin backfill button to generate clips for other existing premium reports; marketing use of tracked clips; VP9/webm variant.

## 2026-08-13 (19) — Instagram connection validation fix (curl-tested)
- PRODUCTION bug: Meta rejected connect with "(#100) Tried accessing nonexisting field (account_type)". Fix in instagram_publish.py put_config: validate via /{ig_id}?fields=id,username,name (no account_type — Meta removed it for business accounts via this edge); require username in response; also store account_name. Publishing flow unchanged. Token stays server-side (db.settings key instagram_publish), GET /config returns only …last4 hint. Confirmed IG business ID: 17841439531129940 (scoutmeplay). USER MUST REDEPLOY to production.

## 2026-08-14 (20) — Production trust fixes: empty snapshots, decorative overlays, report navigation (self-tested E2E in preview)
User reported on PRODUCTION (scoutmeplay.com) after redeploy + new upload: (1) 3 of 4 Scout Evidence snapshot cards showed cream "Moment placeholder" images (huge mistrust), (2) circles/arrows not on the player in "video proofs", (3) confusing premium report navigation (opening second category didn't visibly close first / page landed at bottom).
Fixes:
- REMOVED decorative client-side telestration: SnapshotAnnot SVG overlays (fixed-position arrows/dotted ellipses) deleted from snapshots.jsx SnapFrame, and _annot() call removed from snapshot_card.py share PNG. Only gated server-baked telestration remains — a circle now always means the verified player.
- Placeholder frames are no longer presented as evidence: ensure_video_frames writes a `.ph` sidecar marker + sets frame_placeholder=true; derive.js snapFrameEntries and PremiumReportV2 proofFrames filter them out (UI shows the stylized pitch fallback instead); R2 flush in _persist_video_frames skips .ph-marked files so placeholders can never become durable evidence.
- STRICT policy holds at read time: ensure_video_frames no longer resurrects identity-dropped moments (identity_verified===False + no frame_url → stays text-only).
- NEW admin repair tool: POST /api/admin/reports/{id}/regenerate-evidence (get_current_admin) — restores video from R2 via _ensure_report_video_local, clears placeholder/unverified frame URLs (keeps anchor-locked + verified), then re-runs _persist_video_frames (frames → identity gate → telestration → tracked proof clips → R2 flush). AdminPage Reports tab: RefreshCcw button `admin-regen-evidence-{id}` on premium rows. Tested E2E on preview report f5748eb6: video restored from R2, 5/5 frames verified, proofclip_1 regenerated (coverage 1.0), weak windows honestly gated.
- Navigation: categories.jsx toggleRow — opening a row scrolls its header under the sticky tab bar (verified: techOpen false after opening evidence, evidence row top=113px).
...
## FIX 00A — Tap Authority Contract (2026-06 / session post-audit)
User-directed narrow fix (first approved phase after the A–Q audit; NOT audit Phase 1).
- Root cause: UploadPage.jsx serialization stripped `verify`/`segment` from Scout Mode anchors (MarkerStudio forwarded them correctly); plus `[:10]` caps in thumb-detach, tracker seeds and GT prompt block.
- Fixed: serialization preserves t/box/segment/verify/thumb; backend parses+persists all 13 original anchors (verify+segment kept); `ORIGINAL_ANCHOR_LIMIT=13`; tracker seed intake `MAX_TRACKER_SEEDS=16` (10 normal + 3 verify + 3 doubt, algorithm untouched); GT positions block covers 13 originals (doubt taps stay excluded); `_identity_ref_crops` prefers verify:true crops for the existing 3 evidence slots; `_prioritized_profile_crops` reorders (never enlarges) profile refs so 3 verify crops sit in the existing 5-image budget; diagnostics: anchors_received/normal/verify log + trace, tracker seed_count in track dict.
- Tests: tests/test_fix00a_tap_authority.py 12/12 passed (deterministic, no LLM). iter95 regression 9/10 — 1 pre-existing data-drift failure (test hardcodes reports==3, DB now has 4 shadow reports). Not fixed (out of scope), reported to user.
- LLM call counts and image budgets unchanged (test-asserted). No DO-NOT-TOUCH area modified.
- NOTE: audit report (A–Q) delivered to user this session from /app/memory/AUDIT.md; audit Phase 1 (event_id ledger etc.) still awaiting user choice.
