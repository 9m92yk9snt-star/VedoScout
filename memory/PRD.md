...
- FIX08 — CORRECTION 01 (June 2026), commit 8d602a4:
  - C01 exact contact ms: projection sets event_start_ms/event_end_ms = ledger_contact_ms (FIX01 preserves); verifier corrected_timestamp + track snap are diagnostic/presentation-only for ledger rows (verifier_corrected_timestamp); event-native evidence_time_ms exact; frames extracted at evidence_time_ms/1000.0
  - C02 event_track_locked: set on event-native rows when spatially verified event + FIX04 bbox resolves at event_start_ms (event_track_box persisted); ensure_video_frames event-native branch (exact-ms extraction, no fingerprint window, no placeholder, fail-closed text-only); _verify_enriched_frames skips locked rows (ZERO per-event model calls, no ±2/4/6 re-window); compute_proof_frame_verified accepts event_track_locked as identity (exact-time still mandatory); never anchor_locked semantics
  - C03 secondary scoring scan: verifier schema extended with contact_ms + actor_box; novel scan GOAL/ASSIST must pass el.validate_actor (ACTOR_MATCH) when track usable, else rejected → unresolved counters; no usable track keeps FIX07-C04 verifier-only behavior; promotions unchanged; accepted adds carry exact contact ms + actor_spatial_verified
  - C04 strict assist chain: all 4 chain times int + continuous + start≤pc≤receive≤shot≤goal≤end + pc==contact + within duration; KEY_PASS needs valid continuous pc≤receive≤shot
  - C05 snapshot authority keys strength/noticed/hidden/develop (SnapshotsSection order); derive.js accepts authority moments at any count 1..4 (legacy ≥4); authority + 0 moments → text-only fallback cards (thumb null)
  - C06 no legacy fallback: authoritative_timeline() in both pipelines — model timeline NEVER authority; discovery failed/malformed/unusable track → []; DISCOVERY UNAVAILABLE prompt notice injected
  - Guards updated: FIX02 compute_proof_frame_verified count 3→4
  - Model cost: full-video +1 discovery only; FIX08-caused image/frame identity model calls: 0
  - Tests: FIX08 45 passed (29 + F01–F16); regressions FIX07 87, FIX06 33, FIX05 30, FIX04 38, FIX03 26, FIX02 29, FIX01 29, FIX00B 31, FIX00A 16
