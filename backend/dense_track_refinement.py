...
def refine_window(dense_frames, scene_graph: dict | None, identity_authority: dict | None,
    ...
    ``detector_fn(frame_bgr)`` may be injected for deterministic tests and must
    return ``(people, balls)``.  Production defaults to the existing local
    person+sports-ball detector and therefore performs one detector forward pass
    per dense frame.  No image arrays are retained in the returned structure.
    """
    scene = str(scene_id or "")
    ...
    for dense in frames_iter:
        ...
        people, balls = detector_fn(frame)
